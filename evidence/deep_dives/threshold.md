# Evidence report — parcel `08253290060000`
...
## Deep-dive narrative — archetype: barely subdividable (threshold)
...
**Why this parcel — selection documented:** the brief asked for a top scheme
of 1–2 lots. **No scheme in the 500-parcel screen has 1–2 lots, and the
smallest top scheme anywhere is 4 lots.** This parcel was therefore chosen as
the **thinnest-margin 4-lot top scheme in the screen** — the closest the data
gets to the threshold archetype.

- Parcel `08253290060000` — **BP** (Business Park), **3.41 acres**. The
  pipeline produced **exactly one scheme** (`scheme_00`, geometry-clean):
  **4 lots**, profit **$184,482.75**, margin **34.16%**.

**Why this is the threshold case:**

1. **One scheme, no fallback.** There is no second layout to retreat to if
   the single scheme's assumptions slip — ranking, sensitivity, and option
   value are all absent by construction.
2. **Thinnest cushion in its cohort.** At 34.16% margin on assumption-grade
   economics, small input moves erase the profit: the economics are
   **DEFAULT-fallback** ("Valley-average Wasatch Front finished SFR lot;
   judgment, no comp pulled"), applied to a BP parcel.
3. **Provisional geometry.** The 4 lots were sized against provisional BP
   dimensionals, and — as on every non-MU-11/non-PL parcel — the RuleGraph
   PASSes are all `applicable: false`: **no BP rule has been human-verified**,
   so compliance is entirely unchecked.
4. **Use-allowance pending.** Whether the intended use is allowed in BP under
   21A.33 has not been evaluated (checker not yet integrated).

**Threshold logic, plainly:** a 4-lot, single-scheme, thin-margin parcel is
where screening precision matters most and where this screen has the least of
it. The $184,482.75 is better read as "worth one careful manual look" than as
a viable project — the first verified BP dimensional or the first real comp
could move it either side of zero.
...
- This parcel (`08253290060000`) covers **3.41 acres** in the **BP** district. Zoning district assignment comes from the real parcel/zoning spatial join; dimensional minimums used for screening are **PROVISIONAL** unless the rule is human-verified (see Section 3).
- The pipeline drew **1 subdivision scheme(s)** (1 geometry-clean, 0 flagged). The top-ranked scheme, `scheme_00`, lays out **4 lots** with 329.9 ft of new road and projects **$184,483 profit** at **34.2% margin**.
- RuleGraph check: **1 scheme(s) PASS, 0 FAIL, 0 UNKNOWN** across the 8 rule(s) in the verified store.
  - **None of the store's rules apply to the BP district** (the 8 verified rules cover MU-11 and PL only), so every PASS above means 'nothing in the store to check against' — not 'compliant'. No in-district dimensional rule has been human-verified yet.
...
- Bottom line: the profit numbers are a geometry-and-assumption sketch. Compliance against BP rules has not been checked at all — no in-district rules exist in the verified store yet.
...
| Fact | Value |
| --- | --- |
| parcel_id | `08253290060000` |
| status | `ok` |
| area | 148,650 sqft (3.41 acres) |
| CRS | `local-feet` |
| zoning district | `Business Park (real AGRC/SLC parcel; dimensionals PROVISIONAL except as verified)` |
| zoning config source | PROVISIONAL dimensional assumptions for mass-screen v1. District assignment is real (spatial join, see screen/real_parcels/SOURCES.md). business park illustrative; provisional — **sourced / stated** |
| geometry notes | 1 scheme(s): 1 geometry-clean, 0 flagged; new road 329.9–329.9 ft across schemes |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | `scheme_00` | 4 | 330 | $540,000 | $355,517 | $184,483 | 34.2% | clean |
...
- Reason: not applicable: rule district PL != parcel district BP
...
- Verdict: **PASS**
- Applicable to this scheme: `False`
- Reason: not applicable: rule district MU-11 != parcel district BP
...
Top scheme: `scheme_00` (4 lots).

| Line | Amount |
| --- | --- |
| Revenue | $540,000 |
| — infrastructure cost | $173,198 |
| — soft costs | $150,000 |
| — contingency | $32,320 |
| **Total cost** | **$355,517** |
| **Profit** | **$184,483** |
...
| --- | --- | --- |
| `sale_price_per_lot` | 135000.0 | |
| `road_cost_per_lf` | 525.0 | |
| `soft_costs_fixed` | 150000.0 | |
| `contingency_pct` | 0.1 | |
| `road_length_ft` | 329.9 | |

- Stated source text: derived from (in-memory) (district=DEFAULT, DEFAULT fallback, economics build 2026-09-23); per-value sources inside the economics file
